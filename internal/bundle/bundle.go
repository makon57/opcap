...
// 	outputDir = "internal/bundle/.test/repo/certified-operators"
// )

func List(bundlesRepo string, bundlesDir string) ([]bundle, error) {
	err := gitCloneOrPullBundles(bundlesRepo, bundlesDir)
	if err != nil {
		return nil, err
	}
...
func gitCloneOrPullBundles(bundlesRepo string, bundlesDir string) error {
	// Clone bundle repository, main branch only (should be the default for certified-operators)
	if _, err := git.PlainClone(bundlesDir, false, &git.CloneOptions{
		URL:      bundlesRepo,
		Progress: os.Stdout,
	...
	repo, err := git.PlainOpen(bundlesDir)
	if err != nil {
